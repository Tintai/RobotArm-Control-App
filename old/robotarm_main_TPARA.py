...
def move_machine_e(direction, distance, speed):
    ...
    if ser is not None:
        send_command_text("G91", False)
        command = f"G1 {direction}{distance} F{speed}"
        send_command_text(command, True)
        send_command_text("G90", False)
        #send_command_text("G92", False)
...
def update_position(lines):
    ...
    for line in lines:
        try:
            ...
            match_tpara = re.match(r'TPARA ROT: (-?\d+\.\d+) LOW: (-?\d+\.\d+) HIGH: (-?\d+\.\d+)', line)
            if match_tpara:
...
def run_gcode(loop_var):
    ...
    def check_info_text_for_match(next_index):
        ...
        # Check if the last line contains TPARA, if not, check the second-to-last line
        if "[TPARA]" in info_text_content.splitlines()[-1]:
        ...
        # Check if TPARA appears in the latest line of info_text
        if "[TPARA]" in latest_line:
            print("[TPARA] found in the latest line. Proceeding to the next command.")
